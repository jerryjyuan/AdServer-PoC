...
#-#############################################
# desc: does a query, fetches the first row of an object only, frees resultset
# param: table name, id
# returns: array of fetched results
function query_first_object($table, $id) {
    $out = $this->query_first("SELECT * FROM ".$table." WHERE id = '".$id."'");
    return $out;
}#-#query_first_object()
...
#-#############################################
# desc: deletes an entry from the db
